"""
DG-P3D ResNet
A PyTorch implement of DG-P3D ResNet as described in
'Optimization Planning for 3D ConvNets' - ICML 2021
...
__all__ = ['DG_P3D_ResNet', 'dg_p3d_resnet18', 'dg_p3d_resnet34', 'dg_p3d_resnet50', 'dg_p3d_resnet101', 'dg_p3d_resnet103',
           'dg_p3d_resnet152', 'dg_p3d_resnext50_32x4d', 'dg_p3d_resnext101_32x8d',
           'dg_p3d_wide_resnet50_2', 'dg_p3d_wide_resnet101_2', 'dg_p3d_resnext103_64x4d']
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        self.conv2_t = conv3x1x1(width, width, groups=groups)
...
class Bottleneck_B(Bottleneck):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        super().__init__(inplanes, planes, stride, downsample, groups,
                         base_width, dilation, norm_layer)
        width = int(planes * (base_width / 64.)) * groups
        self.conv2_t = conv3x1x1(width, width, groups=groups, stride_s=stride)
...
class DG_P3D_ResNet(nn.Module):
    ...
    def __init__(self, block, layers, pooling_arch, early_stride=4, num_classes=400, dropout_ratio=0.5, zero_init_residual=False,
        ...
        super(DG_P3D_ResNet, self).__init__()
        if block == BasicBlock:
        ...
        self.maxpool_dual = nn.MaxPool3d(kernel_size=[4, 1, 1], stride=[4, 1, 1], padding=[0, 0, 0])
        self.layer4_dual = self._make_layer([block[(i + sum(layers[:3])) % 3] for i in range(layers[3])], 512,
                                            layers[3], stride=1, dilate=replace_stride_with_dilation[2])
                    ...
                    if (block_order % 3) == 0:  # P3DA
                        for n in range(0, shape[1]):
                            for g in range(0, group):
                                kernel[n + g * shape[1], n, 0, 0, 0] = 0.0
                                kernel[n + g * shape[1], n, 1, 0, 0] = 1.0
                                kernel[n + g * shape[1], n, 2, 0, 0] = 0.0
                ss = k.split('.')
                new_key = k[:-len(ss[-1]) - 1] + '_t.' + ss[-1]
                new_state_dict[new_key] = torch.from_numpy(kernel)
                new_state_dict[new_key.replace('layer4', 'layer4_dual')] = torch.from_numpy(kernel)
...
def _dg_p3d_resnet(arch, block, layers, pooling_arch, image_size=None, **kwargs):
    model = DG_P3D_ResNet(block, layers, pooling_arch, **kwargs)
    return model


@register_model
def dg_p3d_resnet18(pooling_arch, **kwargs):
    ...
    """
    return _dg_p3d_resnet('resnet18', BasicBlock, [2, 2, 2, 2], pooling_arch,
                   **kwargs)


@register_model
def dg_p3d_resnet34(pooling_arch, **kwargs):
    ...
    Args:
        pooling_arch: choose the pooling architecture for last conv/res layer
    """
    return _dg_p3d_resnet('resnet34', BasicBlock, [3, 4, 6, 3], pooling_arch,
                   **kwargs)


@register_model
def dg_p3d_resnet50(pooling_arch, **kwargs):
    ...
    Args:
        pooling_arch: choose the pooling architecture for last conv/res layer
    """
    return _dg_p3d_resnet('resnet50', Bottleneck, [3, 4, 6, 3], pooling_arch,
                   **kwargs)


@register_model
def dg_p3d_resnet101(pooling_arch, **kwargs):
    ...
    Args:
        pooling_arch: choose the pooling architecture for last conv/res layer
    """
    return _dg_p3d_resnet('resnet101', Bottleneck, [3, 4, 23, 3], pooling_arch,
                   **kwargs)
                   

@register_model
def dg_p3d_resnet103(pooling_arch, **kwargs):
    ...
    Args:
        pooling_arch: choose the pooling architecture for last conv/res layer
    """
    return _dg_p3d_resnet('resnet103', Bottleneck, [3, 4, 23, 3], pooling_arch, deep_stem=True,
                   **kwargs)


@register_model
def dg_p3d_resnet152(pooling_arch, **kwargs):
    ...
    Args:
        pooling_arch: choose the pooling architecture for last conv/res layer
    """
    return _dg_p3d_resnet('resnet152', Bottleneck, [3, 8, 36, 3], pooling_arch,
                   **kwargs)


@register_model
def dg_p3d_resnext50_32x4d(pooling_arch, **kwargs):
    ...
    return _dg_p3d_resnet('resnext50_32x4d', Bottleneck, [3, 4, 6, 3],
                   pooling_arch, **kwargs)


@register_model
def dg_p3d_resnext101_32x8d(pooling_arch, **kwargs):
    ...
    return _dg_p3d_resnet('resnext101_32x8d', Bottleneck, [3, 4, 23, 3],
                   pooling_arch, **kwargs)


@register_model
def dg_p3d_resnext103_64x4d(pooling_arch, **kwargs):
    ...
    kwargs['groups'] = 64
    kwargs['width_per_group'] = 4
    return _dg_p3d_resnet('resnext103_64x4d', Bottleneck, [3, 4, 23, 3],
                   pooling_arch, deep_stem=True, **kwargs)


@register_model
def dg_p3d_wide_resnet50_2(pooling_arch, **kwargs):
    ...
    return _dg_p3d_resnet('wide_resnet50_2', Bottleneck, [3, 4, 6, 3],
                   pooling_arch, **kwargs)


@register_model
def dg_p3d_wide_resnet101_2(pooling_arch, **kwargs):
    ...
    """
    kwargs['width_per_group'] = 64 * 2
    return _dg_p3d_resnet('wide_resnet101_2', Bottleneck, [3, 4, 23, 3],
                   pooling_arch, **kwargs)
