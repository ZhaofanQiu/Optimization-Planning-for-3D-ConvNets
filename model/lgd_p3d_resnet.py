"""
LGD-3D ResNet
A PyTorch implement of LGD-3D ResNet as described in
...
__all__ = ['LGD_P3D_ResNet', 'lgd_p3d_resnet18', 'lgd_p3d_resnet34', 'lgd_p3d_resnet50', 'lgd_p3d_resnet101', 'lgd_p3d_resnet103',
           'lgd_p3d_resnet152', 'lgd_p3d_resnext50_32x4d', 'lgd_p3d_resnext101_32x8d',
           'lgd_p3d_wide_resnet50_2', 'lgd_p3d_wide_resnet101_2']
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        # local in
        self.fc1 = nn.Linear(inplanes, planes * self.expansion // 16)
        self.fc2 = nn.Linear(planes * self.expansion // 16, planes * self.expansion)
        # global in
        self.fc3 = nn.Linear(inplanes, planes * self.expansion // 16)
        self.fc4 = nn.Linear(planes * self.expansion // 16, planes * self.expansion)
        # global out
        self.fc5 = nn.Linear(planes * self.expansion, planes * self.expansion // 16)
        self.fc6 = nn.Linear(planes * self.expansion // 16, planes * self.expansion)
    ...
    def forward(self, x):
        ...
        in_local = self.fc2(self.fc1(torch.flatten(self.avgpool(x[0]), 1)))
        in_global = self.fc4(self.fc3(x[1]))
        out_global = self.relu(in_local + in_global)

        out += identity
        out += self.fc6(self.fc5(out_global)).view(out_global.size(0), out_global.size(1), 1, 1, 1)
        out = self.relu(out)
...
class BasicBlock_B(BasicBlock):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        self.conv1_t = conv3x1x1(planes, planes, stride_s=stride)

    def forward(self, x):
        identity = x[0]

        out_s = self.conv1(x[0])
        out_t = self.conv1_t(x[0])
...
class LGD_P3D_ResNet(nn.Module):
    ...
    def __init__(self, block, layers, pooling_arch, early_stride=4, num_classes=400, dropout_ratio=0.5, zero_init_residual=False,
        ...
        super(LGD_P3D_ResNet, self).__init__()
        if block == BasicBlock:
        ...
        self.init_mem = nn.AdaptiveAvgPool3d((1, 1, 1))
        ...
        self.drop_g = nn.Dropout(dropout_ratio, inplace=False)

        self.fc = nn.Linear(self.pool.output_dim, num_classes)

        self.fc_g = nn.Linear(512 * block[0].expansion, num_classes)
        ...
        for m in self.modules():
            if isinstance(m, (Bottleneck, BasicBlock)):
                nn.init.constant_(m.fc6.weight, 0)
    ...
    def _forward_impl(self, x, layer):
        ...
        if not self._deep_stem:
            ...
            x = self.relu3(x)
        x = self.maxpool(x)
        g = torch.flatten(self.init_mem(x), 1)
        ...
        # global path
        x_global = self.fc_g(self.drop_g(xg[1]))
...
def _lgd_p3d_resnet(arch, block, layers, pooling_arch, image_size=None, **kwargs):
    model = LGD_P3D_ResNet(block, layers, pooling_arch, **kwargs)
    return model


@register_model
def lgd_p3d_resnet18(pooling_arch, **kwargs):
    ...
    """
    return _lgd_p3d_resnet('resnet18', BasicBlock, [2, 2, 2, 2], pooling_arch,
                   **kwargs)


@register_model
def lgd_p3d_resnet34(pooling_arch, **kwargs):
    ...
    Args:
        pooling_arch: choose the pooling architecture for last conv/res layer
    """
    return _lgd_p3d_resnet('resnet34', BasicBlock, [3, 4, 6, 3], pooling_arch,
                   **kwargs)


@register_model
def lgd_p3d_resnet50(pooling_arch, **kwargs):
    ...
    Args:
        pooling_arch: choose the pooling architecture for last conv/res layer
    """
    return _lgd_p3d_resnet('resnet50', Bottleneck, [3, 4, 6, 3], pooling_arch,
                   **kwargs)


@register_model
def lgd_p3d_resnet101(pooling_arch, **kwargs):
    ...
    Args:
        pooling_arch: choose the pooling architecture for last conv/res layer
    """
    return _lgd_p3d_resnet('resnet101', Bottleneck, [3, 4, 23, 3], pooling_arch,
                   **kwargs)


@register_model
def lgd_p3d_resnet103(pooling_arch, **kwargs):
    ...
    Args:
        pooling_arch: choose the pooling architecture for last conv/res layer
    """
    return _lgd_p3d_resnet('resnet103', Bottleneck, [3, 4, 23, 3], pooling_arch, deep_stem=True,
                   **kwargs)


@register_model
def lgd_p3d_resnet152(pooling_arch, **kwargs):
    ...
    Args:
        pooling_arch: choose the pooling architecture for last conv/res layer
    """
    return _lgd_p3d_resnet('resnet152', Bottleneck, [3, 8, 36, 3], pooling_arch,
                   **kwargs)


@register_model
def lgd_p3d_resnext50_32x4d(pooling_arch, **kwargs):
    ...
    return _lgd_p3d_resnet('resnext50_32x4d', Bottleneck, [3, 4, 6, 3],
                   pooling_arch, **kwargs)


@register_model
def lgd_p3d_resnext101_32x8d(pooling_arch, **kwargs):
    ...
    return _lgd_p3d_resnet('resnext101_32x8d', Bottleneck, [3, 4, 23, 3],
                   pooling_arch, **kwargs)


@register_model
def lgd_p3d_wide_resnet50_2(pooling_arch, **kwargs):
    ...
    return _lgd_p3d_resnet('wide_resnet50_2', Bottleneck, [3, 4, 6, 3],
                   pooling_arch, **kwargs)


@register_model
def lgd_p3d_wide_resnet101_2(pooling_arch, **kwargs):
    ...
    """
    kwargs['width_per_group'] = 64 * 2
    return _lgd_p3d_resnet('wide_resnet101_2', Bottleneck, [3, 4, 23, 3],
                   pooling_arch, **kwargs)
