...
'Rethinking spatiotemporal feature learning: Speed-accuracy trade-offs in video classification' - ECCV 2018
This implementation is based on https://raw.githubusercontent.com/qijiezhao/s3d.pytorch/master/S3DG_Pytorch.py
...
class STConv3d(nn.Module):
    def __init__(self,in_planes,out_planes,kernel_size,stride,padding=0):
        ...
        self.conv2_t = nn.Conv3d(out_planes,out_planes,kernel_size=(kernel_size,1,1),
        ...
        self.conv2_t.weight.data.normal_(mean=0, std=0.01) # original s3d is truncated normal within 2 std
...
class SepInception(nn.Module):
    def __init__(self, in_planes, out_planes, gating=False):
        ...
        self.bn_planes = [num_out_0_0a, num_out_1_0b, num_out_2_0b, num_out_3_0b]

        # self.bn0 = nn.BatchNorm3d(num_out_0_0a)
        # self.bn1 = nn.BatchNorm3d(num_out_1_0b)
        # self.bn2 = nn.BatchNorm3d(num_out_2_0b)
        # self.bn3 = nn.BatchNorm3d(num_out_3_0b)
...
class S3D(nn.Module):
    ...
    def __init__(self, pooling_arch, num_classes=400, dropout_ratio=0.5, input_channel=3, gating=False, slow=False):
        ...
        self.MaxPool_5a = nn.MaxPool3d(kernel_size=(1, 2, 2), stride=(1, 2, 2), padding=(0, 0, 0))
        ...
        self.block5 = nn.Sequential(
            ...
            self.Mixed_5c)    # (1024, 8, 7, 7)s
        # self.block5_1 = self.MaxPool_5a
        # self.block5_2 = self.Mixed_5b
        # self.block5_3 = self.Mixed_5c
        ...
        #     self.Dropout_0b,
        #     self.Conv_0c)
        self.pool = pooling_arch(input_dim=self.feature_size)
...
@register_model
def s3d(pooling_arch, image_size=None, early_stride=None, **kwargs):
    return S3D(pooling_arch, gating=False, slow=False, **kwargs)


@register_model
def s3dg(pooling_arch, image_size=None, early_stride=None, **kwargs):
    return S3D(pooling_arch, gating=True, slow=False, **kwargs)