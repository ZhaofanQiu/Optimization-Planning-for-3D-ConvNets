...
use_depthwise_conv3d = False

# if use_depthwise_conv3d:
#    from layer.depthwise_conv3d.depthwise_conv3d import depthwise_conv, select_algo_id
...
class Conv3dStaticSamePadding(nn.Conv3d):
    ...
    # Caculation is the same with Conv2dStaticSamePadding
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, image_size=None, **kwargs):
        ...
        assert image_size is not None
        ih, iw = (image_size, image_size) if isinstance(image_size, int) else image_size
        kt, kh, kw = self.weight.size()[-3:]
        sh, sw = self.stride[1], self.stride[2]
        oh, ow = math.ceil(ih / sh), math.ceil(iw / sw)
    ...
    def forward(self, x):
        x = self.static_padding(x)
        # if use_depthwise_conv3d and self.weight.size()[0] == self.groups and x.device.type == 'cuda':
            # if self.alg_id < 0:
                #self.alg_id = select_algo_id(x, self.weight, self.bias, self.weight.size()[-3:], self.stride, self.padding, self.bias is not None)
                
            #x = depthwise_conv(x, self.weight, self.bias, self.weight.size()[-3:], self.stride, self.padding, self.bias is not None, self.alg_id)
        #else:
